...
const Retailer = () => {
  ...
  const handleInterest = async (cropTitle, userId) => {
    try {
      const response = await fetch(
        "http://localhost:3000/api/v1/notifyFarmer",
        {
          method: "POST",
          headers: {
    ...
    <>
      <div className="crop-page">
        ...
        <main className="crop-display">
          ...
          <h4>
            Agro Market is a one-stop where you can find the best prices for
            crops in a nearby location, where you can purchase directly from the
            farmer.
          </h4>
          {loading ? (
            <p>Loading...</p>
            ...
            <div className="card-container">
              {crops.map((crop, index) => (
                <div className="crop-card" key={index}>
                  ...
                  <p>
                    {" "}
                    <span className="style-p">Quantity:</span> {crop.quantity}
                  </p>
                  <p>
                    {" "}
                    <span className="style-p">Price:</span> ₹{crop.price}
                  </p>
                  <p>
                    {" "}
                    <span className="style-p">Seller:</span> {crop.userName}
                  </p>

                  <p>
                    {" "}
                    <span className="style-p">Location:</span>{" "}
                    {crop.userLocation}
                  </p>
